//  Service component for the AWS S3 Media Files LWCs.
//
...
const exif2dec = (exifGeolocation) => {
	const parts = exifGeolocation.split(/[^\d\w\.]+/);
	const decimal = Number(parts[0]) + Number(parts[2])/60.0 + Number(parts[3])/(60.0 * 60.0);
	return (parts[4] === "S" || parts[4] === "W") ? -decimal : decimal;
}
